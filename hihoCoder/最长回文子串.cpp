...
//C没有类型比较操作，比较地址可以利用编译器发现类型不一致错误
#define min(x, y) ({		\
typeof(x) _x = (x);         \
typeof(y) _y = (y);         \
(void) (&_x == &_y);        \
...
// 重新包装原串，合并奇偶情况，中间和开头结尾加#，为防止在扩张中越界，再在开头末尾加入$，@，注意这两个字符是不可以对称的
...
//            int x = plen[2*rmostIdx - i];
//            int y = plen[rmostIdx]-2*(i -rmostIdx);
//            plen[i] = min(x, y);
