...
void updateBSTree(int numIdx, int state, int mytime, int mask = 32){
    ...
    // }
    // // 找到这个模式对应的节点
    // bsTree[cur].stat = state;
    // bsTree[cur].timeStamp = mytime;
...
bool queryBSTree(int finalIdx){
    ...
    //     if(finalIdx & mask){
    //         cur = 2*cur + 1;
    //         if(bsTree[cur].stat != 0){
    //             if(bsTree[cur].timeStamp < curTimeStamp){
    //                 curState = bsTree[cur].stat;
    //                 curTimeStamp = bsTree[cur].timeStamp;
    //             }
    //         }else{
    //             continue ;
    //         }
